...
if [ ! -d "./pub" ]; then
    ...
    if ! curl -o stellar-fe-${TAG}.tar.gz -L https://github.com/caapap/fe/releases/download/${TAG}/stellar-fe-${TAG}.tar.gz; then
        echo "failed to download stellar-fe-${TAG}.tar.gz!"
        exit 1
    fi

    if ! tar zxf stellar-fe-${TAG}.tar.gz; then
        echo "failed to untar stellar-fe-${TAG}.tar.gz!"
        exit 2
    fi
fi
